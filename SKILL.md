...
2. **Long-running commands** — use `rt exec --bg` (any host) or `rt slurm submit` (Slurm hosts). SSH timeouts will kill foreground commands over a few minutes. **A stop from your side — Ctrl-C, a task stop, a closed terminal — kills only the local `ssh`; the remote command keeps running as an orphan.** `rt exec` says so at that moment (exit 128+signal, output marked as a PREFIX) and has no kill plumbing by design (ruled 2026-09-05): anything you may need to stop goes through `--bg`, where `rt logs` names the tmux session to kill; otherwise `pgrep`/`kill` it on the host. **When you kill by pattern through `rt exec`, the pattern must not match the exec itself**: `rt exec` runs your text inside a `bash -c '…'` wrapper whose command line contains the pattern, so `pkill -f '<pattern>'` kills that wrapper too and the output ends as a TRUNCATED prefix without telling you whether the kill worked (measured 2026-09-13). Match the program name (`pgrep -x du`), use the bracket trick (`pkill -f '[d]u -sh …'`), or read `ps` first and kill by PID.
