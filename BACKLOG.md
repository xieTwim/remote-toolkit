...
- [fixed remote-toolkit@PENDING] 2026-08-02 **an unhealthy sync session was classified `active`.** The classifier had no negative case: anything carrying `Identifier:` that did not match `Status:.*Paused` and did not match `Connected: No` was called healthy, so every state Mutagen has that those two strings do not name reported as healthy. The one that actually happens is the entry-count circuit breaker, which reports `Connected: Yes` on BOTH endpoints with `Status: Waiting … for rescan`. Re-confirmed against real Mutagen 0.18.1 2026-08-05. `_sync_status` now classifies from a structured `mutagen sync list --template` probe of `.Paused` / `.LastError` / `.Alpha.Connected` / `.Beta.Connected`, and returns six states rather than four: the new ones are `halted` (a live error — measured NOT sticky, it clears within one rescan interval once the condition clears, so it is a live condition and not a scar) and **`unknown`** (the daemon could not be asked — previously encoded as `none`, i.e. "there is no sync session", a claim about the remote that nothing established). Two consequences were larger than the classification itself: (a) measured 2026-08-05, a flush against a halted session does not fail, it **HANGS** — still running after 15s — so it used to burn `RT_FLUSH_TIMEOUT` and return **3**, "the daemon is still syncing", which `exec` continues on; `_sync_flush` now returns 2 for `halted` and `unknown`. (b) `connect` says plainly that it cannot recover a halted session instead of resuming a no-op and printing "Already connected" — resume is for PAUSED sessions, and the fix is to repair `MUTAGEN_IGNORE` and RECREATE, since ignores apply only at session-create time. `connect` also refuses to create a session while the daemon is unaskable, which could otherwise have made a second session for a profile that already had one. SKILL.md's claim that a breaker halt shows as `sync=paused` was false and is corrected — that recovery path never fired
