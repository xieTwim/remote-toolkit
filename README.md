...
| `rt exec` says `output is TRUNCATED` (exit **125**, or the transport's own status such as **255**) | The stream ended before the remote command reported finishing, so what arrived is a **prefix**, not the answer. 125 means "no outcome was established"; a transport that failed on its own keeps its own status instead. **Do NOT read this as "the command failed", and do not re-run it blind — the remote process usually SURVIVES the local `ssh` dying** (see the SKILL's `--timeout` note: killing the local `ssh` does not reliably kill the remote side). First establish the remote's actual state — `ps -eo pid,etime,cmd \| grep '[y]our-command'` for the process, and a count of what it produces or consumes for progress — then decide. Re-running blind double-executes, which for anything destructive or non-idempotent is the expensive mistake. For large payloads redirect to a file rather than capturing the stream. |
| I stopped a foreground `rt exec` (Ctrl-C, task stop, closed terminal) — is the remote command dead? | **No.** The stop kills only the local `ssh`; the remote command keeps running as an orphan. `rt exec` now says so at that moment (exit 128+signal, the received output marked as a PREFIX) and has no kill plumbing by design — `--bg` is the one stoppable lane (`rt logs` names the tmux session to kill); otherwise `pgrep`/`kill` it on the host. |
| `rt status --all` shows `⚠ LOCAL/REMOTE sync root ABSENT (<dir>)` | The directory that session synchronises no longer exists on that endpoint — the precondition for a wholesale propagation in either direction (the deletion onto the replica, or the replica back onto the remote). rt reports it and changes nothing about what Mutagen does next; establish which side is authoritative before the next cycle (`disconnect` keeps the local replica; `mutagen sync reset` reconciles). `(remote root not established)` means the host did not answer, which is not evidence of absence. |
| A long remote command's progress "stopped" | Check the RIGHT signal before concluding anything. Coarse counters lie: `rm -rf` is depth-first, so a top-level entry disappears only when its whole subtree is gone and the entry count can sit unchanged for hours while the delete runs fine. Count leaves (`find <path> \| wc -l`) twice, minutes apart. And do not read mtimes through `ls --time-style=...` in an `awk`-parsed pipeline — the flag moves the columns, so a positional `$6`/`$7` silently reports a different field. |
| A large `rt exec` payload came back cut off, and the call exited 0 | **Use `rt fetch`, not `exec`.** Neither `rt` nor `ssh` caps the payload, so a cut at ~100 KB is in whatever *captures* rt's stdout, and a pipeline throws rt's exit status away — which is why every one of the four measured callers was deaf to the 125. `rt fetch <local> '<remote-command>'` digests the payload on the remote, verifies what arrived, and renames into place only on a match; on any failure the destination is **not written**. `exec`'s size line and 125 remain as diagnostics. |
| A file "deployed" to the remote but is 0 bytes / truncated | **Use `rt push <local> <remote>`, never `base64 \| rt exec 'base64 -d > f'`** — that pipeline delivered an empty stdin stream 2 of 3 times, `base64 -d` exited 0 on zero bytes, and a GPU evaluation was consumed running an empty script. `push` digests locally, has the remote digest what it received, and installs atomically only on a match. |
| A remote file went EMPTY right after reading it | **Never redirect into a path inside the synced tree**: `rt exec -- 'cat results/X.csv' > results/X.csv` destroys the remote original — the shell truncates the LOCAL target first, and the bidirectional sync propagates the empty file to the remote before the `cat` reads it. Redirect to a scratch path outside the tree, or just let the sync bring the file down (there is no `rt pull`; the sync is continuous). |
| A cleanup keeps un-doing itself / GBs reappear locally | **Data that is not part of the working tree does not belong inside `LOCAL_DIR`.** An ignore list protects the paths you named, and a rename creates a path you did not name (a corpus parked as `results/.X_OLD` matched no ignore and synced 1.78 GB back). Park fallbacks/archives OUTSIDE the synced root; when removing a synced dir, remove on the remote first — the sync is bidirectional, and deleting locally first propagates INTO the copy you were trying to keep. |
| `rt fetch`/`rt push` exits **3** | **Digest mismatch** — the payload was cut or altered in transit. The destination was not written and the incoming file was removed. Distinct from exit **2**, `unverifiable`, which means one side has no `sha256sum`/`shasum` and nothing was established either way. |
